...
FCS_Cycles=float(config.get('FCS','CYCLES'))
...
VG_Config=config.get('FCS','VG_SPAN').split(";")
VDS_Config=float(config.get('FCS','VDS'))
Bursts_config=float(config.get('FCS','BURSTS'))
Max_VDS_Config=float(config.get('FCS','MAX_VDS_EXPECT'))
nPontosmedia=int(config.get('FCS','PONTOS_GRUPOS')) 
Freq_Config=float(config.get('FCS','FREQ'))
#Flat_Region_width_config=config.get('FCS','FLAT_REGION_WIDHT')


print(FCS_Cycles,amostras,
gain,
vg_max_criteria,
vg_min_criteria,
VG_Config,
...
print(local)
data.to_csv(path_or_buf=local +"\\resVg.csv", sep=";", index = False)
Info_teste=pd.DataFrame(Configuracoes,index=[0])
