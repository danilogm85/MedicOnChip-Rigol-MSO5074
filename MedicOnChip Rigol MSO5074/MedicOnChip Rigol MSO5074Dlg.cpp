	...
	, m_results_display(_T(""))
{
	...
	//if (infos[3] != "00.01.03.02.02\n") {			Para testar com o osciloscópio com FW novo
	...
	if (!m_bAquisicaoAtiva)
	{
		...
		if (status_sn_prompt == IDOK || flag_run_all) {
			if (iniciarAquisicao()) {
				...
				/*
				build_log_message("Configurando oscilosc�pio para FCC");
				UpdateData(TRUE);
	...
	if (!m_bAquisicaoAtiva)
	{
		...
		if (status_sn_prompt == IDOK || flag_run_all){
			if (iniciarAquisicao()) {
				...
				*/
				/*
				build_log_message("Configurando oscilosc�pio para FCS");
				UpdateData(TRUE);
				m_receive = tester.log_string.c_str();
				UpdateData(FALSE);
				*/
	...
	if (!m_bAquisicaoAtiva)
	{
		if (m_SNPrompt.DoModal() == IDOK)
		{
			if (iniciarAquisicao()) {
				...
				/*
				build_log_message("Configurando oscilosc�pio para FCP");
				UpdateData(TRUE);
				m_receive = tester.log_string.c_str();
				UpdateData(FALSE);
				*/
		...
		if (trg_status == "STOP\n") {
			...
			if (vg_index < results.vg_vector.size()) {
				if (burst_count < num_bursts) {
					/*
					std::string msg = "Aquisi��o encerrada, BURST " + std::to_string(burst_count) + ", Vg = " + std::to_string(results.vg_vector[vg_index]);
					...
					*/
					...
					if (!fs::exists(path)) {
						/*
						build_log_message("Criando diret�rios");
						UpdateData(TRUE);
						m_receive = tester.log_string.c_str();
						UpdateData(FALSE);*/
					...
					*/
					...
					{/*
						build_log_message("Preparando pr�xima aquisi��o");
						UpdateData(TRUE);
						m_receive = tester.log_string.c_str();
						UpdateData(FALSE);
						*/
					...
					if (vg_index < results.vg_vector.size()) {
						/*
						build_log_message("Iniciando medi��es para o pr�ximo Vg");
						UpdateData(TRUE);
						m_receive = tester.log_string.c_str();
						UpdateData(FALSE);
						*/
				...
				build_log_message("Aquisi��es finalizadas, desligando canais");
				UpdateData(TRUE);
				m_receive = tester.log_string.c_str();
				UpdateData(FALSE);
				*/
				...
				vds_source.stop(1);
				/*
				build_log_message("Chamando Python para an�lise dos dados");
				UpdateData(TRUE);
				m_receive = tester.log_string.c_str();
				UpdateData(FALSE);
				*/
				...
				//const std::string &path = "C:\\Users\\Mediconchip.DESKTOP-K5I25D1\\Desktop\\Reposit�rio Fabrinni\\ResistenciaCanal.py";
				//LPCTSTR blabla = path.c_str();

				//system("C:\\Users\\Mediconchip.DESKTOP-K5I25D1\\Desktop\\Reposit�rio Fabrinni\\ResistenciaCanal.py");
			...
			build_log_message("Aquisi��o em progresso");
			UpdateData(TRUE);
			m_receive = tester.log_string.c_str();
			UpdateData(FALSE);*/
		}
	...
	case ID_TIMER_FCS:
		...
		if (trg_status == "STOP\n") {
			//if (vg_index < results.vg_vector.size()) {
				if (burst_count < num_bursts) {
					/*
					std::string msg = "Aquisi��o encerrada, BURST: " + std::to_string(burst_count);
					build_log_message(msg);
					UpdateData(TRUE);
					m_receive = tester.log_string.c_str();
					UpdateData(FALSE);
...
void CMedicOnChipRigolMSO5074Dlg::Measure_and_save(const vector <unsigned int>& channels, unsigned int bucket_size, std::string raw_path,std::string mean_path, const std::vector <float>& offsets) {
	...
	for (int i = 0; i < readcnt; i++)
		temp[i] = buf[i];
	Ts = 1/atof(temp);		//Per�odo de amostragem
	/*
	std::string srate_path = "";
	for (int i = (mean_path.length() - 1); i > 0;i--) {	//Acha a ultima barra
		if (mean_path[i] ==  '/') {
			for (int j = 0; j < (i+1); j++) {	//Copia string at� a barra, obtendo o path do resultado do teste
	...
	for (int canal = 1; canal < 4; canal++) {
		...
		//S� mede se o canal estiver selecionado no vetor channels
		if (iterador != channels.end()) {
			...
			for (int i = 0; i < tam; i++) {
				...
				if (aux == bucket_size-1) {
					...
					sinal_it++;
					aux = -1;	//O aux � incrementado logo que sai do if, ent�o tem que colcoar -1 para ele zerar daqui 2 linhas
					soma = 0;
				}
				aux++;
			}
	...
	if (check_response_loop) {
		...
		while (buf != expected_msg + "\n") {
			...
			if (loop_count == COMM_LOOP_LIMIT) {
				AfxMessageBox(_T("Erro: Problema de comunica��o com o oscilosc�pio. Reconecte a USB."));
				viClose(vi);
				sleep_for(milliseconds(20));
				viClose(defaultRM);
				sleep_for(milliseconds(20));
...
void CMedicOnChipRigolMSO5074Dlg::OnBnClickedButtonFcsAlt()
{
	if (m_SNPrompt.DoModal() == IDOK)
	{
		...
		std::string fcc_path_str = CStringA(fcc_path);	//Jogando na result_path, que � string, porque a fun��o de baixo s� aceita string
		...
		if (fs::exists(fcc_path_str)) {
			...
			UpdateData(TRUE);
			m_receive = "Esse chip ainda n�o foi testado no FCC";
			UpdateData(FALSE);
			return;
		}
			...
			if (aq_status) {
				if (!flag_scale_set_status) {
					...
					if (Freq_Iterator == 0) {	//P/ multiplas frequencia
						...
						CopyConfig();
						std::ifstream inputFile(vg_values_path, std::ios::binary); // Abre o arquivo de origem em modo bin�rio
						std::ofstream outputFile(result_path + "vg_values.ini", std::ios::binary); // Abre o arquivo de destino em modo bin�rio
						outputFile << inputFile.rdbuf(); // Copia o conte�do do arquivo de origem para o arquivo de destino
					}
					//Mostra os gr�ficos
					for (int i = 0; i < m_numCanais; i++)
						m_pwndGraficoCanal[i].ShowWindow(SW_SHOW);
					...
					trigger_parameters.source = "CHAN3";
					tester.send_trigger_parameters(trigger_parameters);
					//tester.triggerMode_FCP();//qlq coisa vai nessa fun��o e muda trigger para Normal
					vds_source.write_parameters_to_osc(results_fcp.vds_source_params);
					vg_source.write_parameters_to_osc(results_fcp.vg_source_params);
				...
				//Medida r�pida para definir escala vertical
...
{
	std::ifstream inputFile("config.ini", std::ios::binary); // Abre o arquivo de origem em modo bin�rio
	std::ofstream outputFile(result_path+"config.ini", std::ios::binary); // Abre o arquivo de destino em modo bin�rio
	outputFile << inputFile.rdbuf(); // Copia o conte�do do arquivo de origem para o arquivo de destino
	return;
}